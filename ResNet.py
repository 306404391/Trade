...
from keras.layers import Conv2D, AveragePooling2D, MaxPooling2D, ZeroPadding2D, GlobalMaxPooling2D, Dropout
...
class ResNet:
    @staticmethod
    def residual_module(x, K, stride, chanDim, reduce=False, reg=0.01, bnEps=2e-5, bnMom=0.99):#结构参考Figure 12.3右图,引入了shortcut概念，是主网络的侧网络
        """
    ...
    @staticmethod
    def build(width, height, depth, classes, stages, filters, reg=0.01, bnEps=2e-5, bnMom=0.99, dataset="cifar"):
        ...
        if dataset == "cifar":
            ...
            x = Conv2D(filters[0], (7, 7), strides=(4,4), use_bias=False, padding="same", kernel_regularizer=l2(reg), activation='relu')(x)
        ...
        x = Activation("relu")(x)
        #x = Conv2D(filters[-1], (1, 1), strides=(2,2), use_bias=False, kernel_regularizer=l2(reg))(x)
